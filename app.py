...
def model_prediction(test_image):
    ...
    image = tf.keras.preprocessing.image.load_img(test_image, target_size=(224, 224))  # Changed to 224x224
    input_arr = tf.keras.preprocessing.image.img_to_array(image)
    input_arr = np.array([input_arr])  # Convert single image to batch
    input_arr = input_arr.astype(np.float32) / 255.0  # Normalize the input
